...
class PubMed():
	...
	def get_pubmed_article(self,item):
		...
		try:
			...
			if 'PubmedArticle' in json_string['PubmedArticleSet'].keys(): 
				...
				article['title'] = json_string['PubmedArticleSet']['PubmedArticle']['MedlineCitation']['Article']['ArticleTitle'].encode('utf-8','ignore')
	...
	def get_pubmed_full_article(self,item):
	    ...
	    try:
	        ...
	        req = urllib2.Request(self.__cfg.pubmedFTP + article['file'])
	...
	def retrieve_file_list(self):
	    ...
	    result = None
	    try:
	        req = urllib2.Request(self.__cfg.pubmedFTPList)
	        f = urllib2.urlopen(req)
